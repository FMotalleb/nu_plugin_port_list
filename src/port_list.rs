...
use nu_protocol::{Category, ErrorLabel,  LabeledError, PipelineData, record, Record, Signature, Span, Value};
    ...
    fn load_process_info_into(rec : &mut Record ,items: &Vec<u32>,skip: bool, span: Span,process_list: &HashMap<String, &Process>) -> Record {
        ...
        for i in items.iter() {
            ...
            if let Some(process_info) =  process {
                ...
                rec.push("process_environments", Value::list(Self::map_environments(process_info.environ(),span), span))
            }
            break;
